...
func MakeArrCap(t typ.Type, len, cap int) (Arr, error) {
	...
	res := abstractArr{t.Next(), make(List, len, cap)}
...
type abstractArr struct {
	elem typ.Type
	List
}

func (a abstractArr) Typ() typ.Type  { return typ.Arr(a.elem) }
func (a abstractArr) Elem() typ.Type { return a.elem }
func (a abstractArr) SetIdx(i int, el Lit) (err error) {
	if el == nil {
		el = Null(a.elem)
	} else {
